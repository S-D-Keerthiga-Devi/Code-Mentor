import React from "react";
import { Link } from "react-router-dom";
import { DocumentArrowUpIcon, ChartBarIcon } from "@heroicons/react/24/outline";
import { UserButton } from "@clerk/clerk-react";
import Navbar from "../../components/Navbar";
import Footer from "../../components/Footer";

const InstructorDashboard = () => {
    return (
        <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col">
            <Navbar />

            <div className="flex-1">
                <div className="max-w-7xl mx-auto p-6 md:p-8 pt-8">
                    <div className="mb-8">
                        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">
                            Instructor Dashboard
                        </h1>
                        <p className="text-gray-600">Manage your course materials and track student progress</p>
                    </div>

                    <h2 className="text-2xl font-bold mb-6 text-gray-800">Instructor Tools</h2>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* Upload Materials Card */}
                        <Link
                            to="/instructor/materials"
                            className="bg-white p-6 rounded-xl border border-gray-200 hover:border-purple-500 hover:shadow-lg transition-all group"
                        >
                            <div className="flex items-center justify-between mb-4">
                                <div className="bg-purple-50 p-3 rounded-lg">
                                    <DocumentArrowUpIcon className="h-8 w-8 text-purple-600" />
                                </div>
                                <span className="text-xs font-semibold bg-green-100 text-green-700 px-2 py-1 rounded">
                                    Active
                                </span>
                            </div>
                            <h3 className="text-xl font-bold mb-2 text-gray-900 group-hover:text-purple-600 transition-colors">
                                Manage Course Materials
                            </h3>
                            <p className="text-gray-500 text-sm">
                                Upload and manage PDF materials for your students' AI assistant.
                            </p>
                        </Link>

                        {/* Student Progress (Placeholder) */}
                        <div className="bg-white p-6 rounded-xl border border-gray-200 opacity-75 cursor-not-allowed">
                            <div className="flex items-center justify-between mb-4">
                                <div className="bg-pink-50 p-3 rounded-lg">
                                    <ChartBarIcon className="h-8 w-8 text-pink-600" />
                                </div>
                                <span className="text-xs font-semibold bg-gray-100 text-gray-500 px-2 py-1 rounded">
                                    Coming Soon
                                </span>
                            </div>
                            <h3 className="text-xl font-bold mb-2 text-gray-900">Student Progress</h3>
                            <p className="text-gray-500 text-sm">
                                Track how students are interacting with the course materials and AI.
                            </p>
                        </div>
                    </div>
                </div>
            </div>

            <Footer />
        </div>
    );
};

export default InstructorDashboard;
